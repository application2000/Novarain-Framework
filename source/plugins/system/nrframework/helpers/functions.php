...
class NRFrameworkFunctions
{
    ...
    /**
	 *  Encodes into JSON keeping the forward slashes unEscaped
	 *  for the URLs to stay intact
	 *
	 *  @param   array|object  $value  	The resource to be encoded into JSON
	 *
	 *  @return  string          		The returned JSON string
	 */
	public static function unEscapeJSONSlashes($value='')
	{
		...
		// Remove the escaped forward slashes
		$unescapedJSON = preg_replace_callback('/\\\u(\w{4})/', function ($matches)
		{
	...
	/**
	 *  Inserts a new key/value before or after the key in the array
	 *
	 *  @param   string  $key        The key to insert after or before
	 *  @param   array   &$array     An array to insert into
	 *  @param   string  $new_key    The new key to insert
	 *  @param   string  $new_value  The new value to insert
	 *
	 *  @return  mixed               The new array if the key exists or FALSE otherwise
	 */
	public static function arrayInsert($key, array &$array, $new_key, $new_value, $direction = 'after')
	{
