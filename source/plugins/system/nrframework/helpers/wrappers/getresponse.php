...
class NR_GetResponse
{
	...
	private function call($api_method = null, $http_method = 'GET', $params = array())
	{
		...
		$options->set('headers.X-Auth-Token', 'api-key ' . $this->api_key);
		$options->set('headers.Accept-Encoding', 'gzip,deflate');
		$options->set('userAgent', 'PHP GetResponse Client 0.0.2 for Joomla');
		$options->set('timeout', $this->timeout);

		if (!empty($this->enterprise_domain))
		{
			$options->set('headers.X-Domain', $this->enterprise_domain);
		}

		if (!empty($this->app_id))
		{
			$options->set('headers.X-APP-ID', $this->app_id);
		}
